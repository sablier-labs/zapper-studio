...
import { SablierV2LockupDynamicV21 } from '../contracts/viem';
...
const category = CoreCategory.LOCKUP_DYNAMIC_21;
const contractAddress = contracts[chain][category].address;

@PositionTemplate()
export class BinanceSmartChainSablierV2LockupDynamic2ContractPositionFetcher extends ContractPositionTemplatePositionFetcher<
  SablierV2LockupDynamicV21,
  DefaultDataProps,
...
> {
  ...
  getContract(address: string) {
    return this.sablierV2ContractFactory.sablierV2LockupDynamicV21({ address, network: this.network });
  }
  ...
  async getTokenDefinitions({
    definition,
  }: GetTokenDefinitionsParams<SablierV2LockupDynamicV21, SablierStreamContractPositionDefinition>) {
    return [
      {
  ...
  async getLabel({ contractPosition }: GetDisplayPropsParams<SablierV2LockupDynamicV21, DefaultDataProps>) {
  ...
  async getBalances(address: string): Promise<ContractPositionBalance<DefaultDataProps>[]> {
    ...
    const contract = this.sablierV2ContractFactory.sablierV2LockupDynamicV21({
