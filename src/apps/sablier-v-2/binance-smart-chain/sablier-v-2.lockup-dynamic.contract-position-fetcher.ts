...
import { SablierV2LockupDynamicV20 } from '../contracts/viem';
...
const category = CoreCategory.LOCKUP_DYNAMIC_20;
const contractAddress = contracts[chain][category].address;

@PositionTemplate()
export class BinanceSmartChainSablierV2LockupDynamicContractPositionFetcher extends ContractPositionTemplatePositionFetcher<
  SablierV2LockupDynamicV20,
  DefaultDataProps,
...
> {
  ...
  getContract(address: string) {
    return this.sablierV2ContractFactory.sablierV2LockupDynamicV20({ address, network: this.network });
  }
  ...
  async getTokenDefinitions({
    definition,
  }: GetTokenDefinitionsParams<SablierV2LockupDynamicV20, SablierStreamContractPositionDefinition>) {
    return [
      {
  ...
  async getLabel({ contractPosition }: GetDisplayPropsParams<SablierV2LockupDynamicV20, DefaultDataProps>) {
  ...
  async getBalances(address: string): Promise<ContractPositionBalance<DefaultDataProps>[]> {
    ...
    const contract = this.sablierV2ContractFactory.sablierV2LockupDynamicV20({
