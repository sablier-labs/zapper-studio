...
import { SablierV2LockupLinearV21 } from '../contracts/viem';
...
const category = CoreCategory.LOCKUP_LINEAR_21;
const contractAddress = contracts[chain][category].address;

@PositionTemplate()
export class BinanceSmartChainSablierV2LockupLinear2ContractPositionFetcher extends ContractPositionTemplatePositionFetcher<
  SablierV2LockupLinearV21,
  DefaultDataProps,
...
> {
  ...
  getContract(address: string) {
    return this.sablierV2ContractFactory.sablierV2LockupLinearV21({ address, network: this.network });
  }
  ...
  async getTokenDefinitions({
    definition,
  }: GetTokenDefinitionsParams<SablierV2LockupLinearV21, SablierStreamContractPositionDefinition>) {
    return [
      {
  ...
  async getLabel({ contractPosition }: GetDisplayPropsParams<SablierV2LockupLinearV21, DefaultDataProps>) {
  ...
  async getBalances(address: string): Promise<ContractPositionBalance<DefaultDataProps>[]> {
    ...
    const contract = this.sablierV2ContractFactory.sablierV2LockupLinearV21({
